...
import { Cartographic, Math, SceneTransforms } from 'cesium';
...
export class CoordinateConverter {
  ...
  static cartesian3ToLatLon(cartesian3: Cartesian3, ellipsoid?: any): {lon: number, lat: number; height: number} {
    ...
    return {
      lon: Math.toDegrees(cart.longitude),
      lat: Math.toDegrees(cart.latitude),
      height: cart.height
    };
  }
  ...
  midPointToCartesian3(first: { latitude: number, longitude: number }, second: { latitude: number, longitude: number }) {
    const toDeg = (rad: number) => Math.toDegrees(rad);
    ...
    return CesiumCartesian3.fromDegrees(middlePoint.lon, middlePoint.lat);
  }
  ...
  middlePointByScreen(position0: Cartesian3, position1: Cartesian3): Cartesian3 {
    ...
    const middleScreenPoint =
      new CesiumCartesian2((screenPosition2.x + screenPosition1.x) / 2.0, (screenPosition2.y + screenPosition1.y) / 2.0);
