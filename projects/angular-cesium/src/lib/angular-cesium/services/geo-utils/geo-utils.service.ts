...
export class GeoUtilsService {
  static pointByLocationDistanceAndAzimuth(currentLocation: any, meterDistance: number, radianAzimuth: number, deprecated?) {
    ...
    const cartographicLocation =
      currentLocation instanceof CesiumCartesian3 ? Cartographic.fromCartesian(currentLocation) : currentLocation;
    const cartesianLocation =
      currentLocation instanceof CesiumCartesian3
        ? currentLocation
        : CesiumCartesian3.fromRadians(currentLocation.longitude, currentLocation.latitude, currentLocation.height);
  ...
  static _pointByLocationDistanceAndAzimuth(cartographicLocation: any, distance: number, radianAzimuth: number) {
    ...
    return CesiumCartesian3.fromRadians(destinationLon, destinationLat);
  }

  static distance(pos0: Cartesian3, pos1: Cartesian3): number {
    return CesiumCartesian3.distance(pos0, pos1);
  }
  ...
  static addDeltaToPosition(position: Cartesian3, delta: Vec3, updateReference = false): Cartesian3 {
    if (updateReference) {
      ...
      const cartesian = CesiumCartesian3.fromRadians(cartographic.longitude, cartographic.latitude, cartographic.height);
      position.x = cartesian.x;
      position.y = cartesian.y;
      position.z = cartesian.z;
      return position;
    } else {
      const cartesian = new CesiumCartesian3(position.x + delta.x, position.y + delta.y, position.z + delta.z);
      const cartographic = Cartographic.fromCartesian(cartesian);
      cartographic.height = 0;
      return CesiumCartesian3.fromRadians(cartographic.longitude, cartographic.latitude, cartographic.height);
    }
  }

  static middleCartesian3Point(position0: Cartesian3, position1: Cartesian3) {
    return new CesiumCartesian3(position1.x - position0.x / 2, position1.y - position0.y / 2, position1.z - position0.z / 2);
  }
